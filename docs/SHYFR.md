...
- `content/shyfr/` — публічний каталог: один файл на категорію, у записі лише `text` і `source`.
- `KOBZA_LEADERBOARD` — наявний Cloudflare KV namespace. Дані «Шифру» ізольовані префіксом `shyfr:`; додаткова база не потрібна.
...
Це всі дозволені поля. Посилання після ` | ` необов’язкове; назва джерела обов’язкова. ID і порядковий номер створюються автоматично. Відсоток прихованих літер залежить лише від номера рівня: 30%, 40%, 50%, 60%, 70%, 78%, 84%, 88%, 92%, потім зростає на 1% до максимуму 96%.

Перевірка й генерація серверного індексу:
...
Worker читає каталог з ключа `shyfr:content:catalog`. Публічний валідатор не дозволяє випадково опублікувати платний текст.
...
3. Налаштуйте variables із `.env.example`, перевірте `SHYFR_MINI_APP_URL`, потім розгорніть Worker звичним для цього сайту способом.
