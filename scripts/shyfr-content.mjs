...
const GENERATED_INDEX = path.join(ROOT, 'api', 'shyfr-content.generated.js');
...
export async function buildShyfrIndex({ output = GENERATED_INDEX } = {}) {
  const result = await validateShyfrContent();
  if (result.errors.length) throw new Error(result.errors.join('\n'));
  const source = `// Generated by scripts/shyfr-content.mjs. Do not edit manually.\nexport const shyfrCategories = ${JSON.stringify(result.categories, null, 2)};\n\nexport const shyfrLevels = ${JSON.stringify(cleanLevels(result.levels), null, 2)};\n`;
  await writeFile(output, source, 'utf8');
  return { categories: result.categories.length, levels: result.levels.length, warnings: result.warnings };
}
...
async function main() {
  ...
  if (command === 'build') {
    const result = await buildShyfrIndex();
    result.warnings.forEach(warning => console.warn(`WARN ${warning}`));
    console.log(`Шифр: створено індекс ${result.categories} категорій, ${result.levels} рівнів.`);
    return;
  }
