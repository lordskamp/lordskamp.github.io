...
test('переноси та іноземні символи залишаються відкритими', () => {
  const substitution = createSubstitution('foreign-literals');
  const tokens = encodePhrase('Тест in the sky\nOK!', substitution);
  assert.equal(tokens.filter(token => token.type === 'letter').length, 4);
  assert.equal(tokens.filter(token => token.type === 'literal').map(token => token.value).join(''), ' in the sky\nOK!');
});
